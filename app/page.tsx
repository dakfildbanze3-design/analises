...
import { useOfflineCollection } from '@/src/hooks/useSocialData';
...
export default function Home() {
  ...
  return (
    ...
    >
      ...
      <section className="bg-[#1a1a1a]/80 backdrop-blur-md pt-2 pb-2 overflow-x-auto hide-scrollbar flex gap-2 pl-3 z-30 sticky top-12 w-full border-none">
        {categories.map((cat) => (
          <div 
            key={cat}
            onClick={() => setActiveCategory(cat)}
            className={`flex-shrink-0 px-4 py-1.5 rounded-[3px] text-[0.6875rem] font-black uppercase tracking-widest cursor-pointer transition-all duration-300
              ${activeCategory === cat ? 'bg-white text-black shadow-lg scale-105' : 'bg-white/10 text-white/50 hover:bg-white/20'}
            `}
          >
            {cat}
          </div>
                  ...
                  <React.Fragment key={product.id}>
                        ...
                        <div className="flex overflow-x-auto hide-scrollbar gap-[4px] pb-2">
                          {videosSlice.map((videoProduct: any) => (
                            <div 
                              ...
                              onClick={() => router.push(`/short/${videoProduct.id}`)}
                              className="relative flex-shrink-0 w-[calc(50vw-4px)] md:w-[170px] h-[340px] rounded-[10px] overflow-hidden cursor-pointer group bg-surface-container shadow-sm"
                            >
                              {(() => {
                    ...
                    {(index + 1) % 5 === 0 && videoProducts[Math.floor(index / 5) % videoProducts.length] && (() => {
                      ...
                      return (
                        ...
                        >
                          <div 
                            onClick={() => router.push(`/short/${v.id}`)}
